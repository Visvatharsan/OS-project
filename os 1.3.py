...
class EnhancedResourceAllocationSimulator:
    ...
    def add_process(self):
        process = simpledialog.askstring("Add Process", "Enter process name:", parent=self.root)
        if process:
    ...
    def add_resource(self):
        resource = simpledialog.askstring("Add Resource", "Enter resource name:", parent=self.root)
        if not resource:
            return
        instances = simpledialog.askinteger("Resource Instances", "Enter number of instances:", 
    ...
    def request_resource(self):
        process = simpledialog.askstring("Request Resource", "Enter process name:", parent=self.root)
        resource = simpledialog.askstring("Request Resource", "Enter resource name:", parent=self.root)
        instances = simpledialog.askinteger("Instances", "Request amount:", parent=self.root, minvalue=1)
        ...
        if self.graph.nodes[resource]['available'] >= instances:
            ...
            # Not enough available, so add a waiting (request) edge (from process to resource)
    ...
    def release_resource(self):
        process = simpledialog.askstring("Release Resource", "Enter process name:", parent=self.root)
        resource = simpledialog.askstring("Release Resource", "Enter resource name:", parent=self.root)
        instances = simpledialog.askinteger("Instances", "Release amount:", parent=self.root, minvalue=1)
        ...
        # Update the allocation edge: subtract the released instances.
        ...
        # Check for waiting requests (blue edges) for this resource.
        waiting_edges = [
        ...
        if waiting_edges and self.graph.nodes[resource]['available'] > 0:
            ...
            # Allocate one instance to the waiting process:
            self.graph.nodes[resource]['available'] -= 1
            # Remove the waiting request edge and add an allocation edge.
            self.graph.remove_edge(waiting_process, resource)
            # Add allocation edge with a temporary red color to indicate the recent allocation.
    ...
    def draw_edges(self):
        for u, v, data in self.graph.edges(data=True):
            ...
            # For allocation edges, use arrow "last"; for request edges, arrow "first".
            arrow = "last" if data.get('type') == 'allocation' else "first"
            # Use the 'color' attribute if available; otherwise, default to green for allocation and blue for request.
            if 'color' in data:
                edge_color = data['color']
            else:
                edge_color = "green" if data.get('type') == 'allocation' else "blue"
            
            self.canvas.create_line(x1, y1, x2, y2, arrow=arrow, fill=edge_color, width=2,
